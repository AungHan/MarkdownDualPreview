...
- **`extension.ts`** — activates once, creates `PreviewManager`, registers `markdownDualPreview.open`/`exportHtml` commands. Both share `resolveTargetDocument()`: resolves an optional `resource: vscode.Uri` argument (passed by editor-title-bar/context-menu invocation, which may target a non-active editor group) via `openTextDocument`, falling back to `activeTextEditor?.document` when invoked with no argument (Command Palette, keybinding).
- **`export/exportHtml.ts`** — renders the doc with the same stateless `Renderer` used for preview into a dependency-free standalone HTML file (no `<script>`, `script-src 'none'` CSP). Prompts embed-vs-link images via `showQuickPick` (`file:` docs only; dismiss = link, not abort). Dark-theme hljs CSS is re-scoped to the export's fixed `vscode-light` body class and gated behind `@media (prefers-color-scheme: dark)`. Images over `MAX_EMBED_BYTES` (5 MB) fall back to `file://` instead of embedding. Custom CSS (below) is inlined last.
- **`preview/customCss.ts`** — `resolveCustomCssFromConfig()`/`resolveCustomCss()` turn `markdownDualPreview.customCss` entries into absolute paths (`.css`-only, remote URLs rejected, dedup case-folded on win32/darwin); `loadCustomCss()` reads+concatenates them (1 MB/file cap). `formatCustomCssWarning()` gives preview and export one consistent rejected-entry message. Deliberately read host-side and pushed as CSS text (not a `<link>`) — avoids granting any `localResourceRoots` and needs no CSP change.
- **`preview/previewManager.ts`** — owns open previews (capped via `maxPreviews`), debounces live updates 300 ms, re-keys panels on rename. Re-pushes custom CSS on `ready`, on config change, and when a resolved custom-CSS file is saved; warns on a rejected entry only once per distinct rejected set (`lastWarnedCustomCssRejection`), not once per panel's `ready` event.
